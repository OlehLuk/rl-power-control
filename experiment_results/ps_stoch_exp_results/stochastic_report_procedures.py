...
def report_const_experiment(name):
    figsize(15,20)
    ...
    for i, subf in enumerate(subfolder_names):
        ...
        plt.title("Constant action {}. MSE={:.4f}, std={:.4f}".format(subf, df_mses[i].mean()[0], df_mses[i].std()[0]))
        plt.legend()
...
def plot_sys_behaviour(name, n_runs=5, steps=[0,-1]):
    figsize(15,20)
    n_runs = 5
    ...
    for i in range(n_runs):
        for j, step in enumerate(steps):
            ...
            plt.plot(df_us[i].index.values, df_us[i].iloc[:, step].values, 'red', label="Actual voltage (u)")
            plt.plot(df_us[i].index.values,  df_ps[i].iloc[:, step].values, 'gray', label="P_ref")
            if step != -1:
