...
def report_experiment(name, plot_perf_sigma=-1, fig_size=(9, 6), exp_name="", var_name="MSE", n_exp=5):
    ...
    perf, sigma = print_exec_summary(df_ep_ls, df_ex_ts, df_expl_perf)
...
def report_const_experiment(name, old=False, fig_size=(12, 20)):
    ...
    for i, subf in enumerate(subfolder_names):
        ...
        avg_mse = df_mses[i].mean()
        std_mse = df_mses[i].std()
        plt.title("Constant action {}. MSE: avg={:.4f}, std={:.4f}".format(subf, avg_mse, std_mse))
        plt.legend()


def print_exec_summary(df_ep_ls, df_ex_ts, df_expl_perf):
